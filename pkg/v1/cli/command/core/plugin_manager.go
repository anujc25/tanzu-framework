		...
		if config.IsFeatureActivated(config.FeatureContextAwareCLIForPlugins) {
			...
			for _, p := range availablePlugins {
				data = append(data, []string{p.Name, p.Description, p.Scope,
					p.Source, getInstalledOrAvailablePluginVersion(p), p.Status})
			}
...
// getInstalledOrAvailablePluginVersion return installed plugin version if plugin is installed
// if not installed it returns available recommanded plugin version
func getInstalledOrAvailablePluginVersion(p plugin.Discovered) string {
