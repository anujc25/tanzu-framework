...
func AvailablePluginsFromLocalSource(localPath string) ([]plugin.Discovered, error) {
	...
	if err != nil {
		if _, err := os.Stat(filepath.Join(localPath, ManifestFileName)); !errors.Is(err, os.ErrNotExist) {
			log.Warningf("Legacy config directory structure detected, trying to list plugins from legacy config directory structure")
			localStandalonePlugins, err = DiscoverPluginsFromLocalSourceWithLegacyDirectoryStructure(localPath)
			if err != nil {
				log.Warningf("Unable to discover standalone plugins from legacy directory structure, %v", err.Error())
			}
		} else {
...
func InstallPluginsFromLocalSource(pluginName, version, localPath string) error {
	...
	if err != nil {
		...
		if _, err2 = os.Stat(filepath.Join(localPath, ManifestFileName)); !errors.Is(err2, os.ErrNotExist) {
			log.Warningf("Legacy config directory structure detected, trying to install plugins from legacy config directory structure")
			// return InstallPluginsFromLocalSourceWithLegacyDirectoryStructure(pluginName, version, localPath)
			// Get discovered plugin resource from legacy directory structure
			plugins, err2 = DiscoverPluginsFromLocalSourceWithLegacyDirectoryStructure(localPath)
		}
		if err2 != nil {
			return errors.Wrap(kerrors.NewAggregate([]error{err, err2}), "unable to discover plugins")
		}
	}
